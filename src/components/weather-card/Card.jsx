...
const Card = (props) => {
    ...
    try{
        return (
            <div className="flex flex-col items-center justify-center p-3 text-white gap-2">
                ...
                <div className="w-3/4 md:w-fit flex md:flex-row md:gap-2 gap-4 flex-col justify-between">
                    {forecastData.forecastday.map((day, ix) => (
                        <div className="flex flex-col justify-center items-center p-3 bg-slate-200/40 rounded-lg" key={ix}>  
                            <h3 className='text-lg font-medium tracking-wide'>{ ix === 0 ? 'Tomorrow' : (new Date(day.date_epoch * 1000).toLocaleString('en-US', { weekday: 'long' })) }</h3>
                            <div className="flex flex-row justify-center items-center p-3">
